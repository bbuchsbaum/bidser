...
### Preferred Queries for New Code

For new workflows, prefer `query_files()`. It keeps the same general idea as
`search_files()`, but makes match semantics explicit and lets you control
whether entities must actually be present and whether to search raw data,
derivatives, or both.

```{r}
# Exact entity matching for reproducible filters
...
# Regex entity matching when selecting multiple values
...
# Require the queried entity to be present on returned files
...
If you have derivatives indexed, `scope` lets you be explicit about where to
search:
...
`search_files()` stays available for compatibility and ad hoc flexible queries:
