...
Thank you for your interest in contributing to bidser! This document
outlines the development workflow and continuous integration setup.
...
This package uses GitHub Actions for continuous integration with the
following workflows:

### Core Workflows

- **R-CMD-check.yaml**: Comprehensive package checking across multiple
  operating systems (Ubuntu, macOS, Windows) and R versions (devel,
  release, oldrel-1)
- **test-coverage.yaml**: Runs test suite and uploads coverage reports
  to Codecov
- **lint.yaml**: Code style checking using the `lintr` package

### Documentation Workflows

- **pkgdown.yaml**: Builds and deploys the package website to GitHub
  Pages
- **render-readme.yaml**: Automatically updates README.md when
  README.Rmd changes
...
The package uses BIDS example datasets for testing and documentation.
These are cached to improve performance:

- Don’t call [`unlink()`](https://rdrr.io/r/base/unlink.html) on dataset
  paths in examples
- Use
  [`get_example_bids_dataset()`](https://bbuchsbaum.github.io/bidser/reference/get_example_bids_dataset.md)
  which has intelligent caching
- Clear cache if needed with
